...
import ubitrack.SWIGTYPE_p_unsigned_char;
...
//		System.out.println("Received image..." + _count);
		if ((_background != null) && (_count % (SKIP_FRAMES + 1) == SKIP_FRAMES)) {			
			SWIGTYPE_p_unsigned_char data_ptr = image.getImageData();
			int calcSize = _width * _height;
			for (int i = 0; i < calcSize; i++) {
				byte b = (byte) ubitrack.unsignedByteArray_getitem(data_ptr, i);
				_imgData[i] = b;
			}
			/*
			 * Part of the better solution that does not work: 
			...
			*/
			...
			_background.setImage(new ImageComponent2D(
					ImageComponent2D.FORMAT_CHANNEL8, _bgImage, true, FLIP_Y));
